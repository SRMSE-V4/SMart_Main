...
def reg_search(query):
	...
	for i in range(0,len(rquery)):
		ans=[]
		neo_query1="match (n) where n.name=~ '(?i).*%s.*' and toFloat(toFloat(%s)/(n.len))>=0.5 return n"%(rquery[i],len(rquery))
...
def main(query):
	...
	result={}
	#print query
	query=query.replace("national highway ","nh")
	query=re.sub(r'nh\s','nh',query)
